...
export default function MatchDetailPage() {
  ...
  const getRiskBadge = (level: string) => {
    ...
    if (lowerLevel === "high" || lowerLevel === "merah" || lowerLevel === "tinggi") return "bg-red-100 text-red-700 border-red-200";
    if (lowerLevel === "medium" || lowerLevel === "kuning" || lowerLevel === "sedang") return "bg-yellow-100 text-yellow-700 border-yellow-200";
    if (lowerLevel === "low" || lowerLevel === "hijau" || lowerLevel === "rendah") return "bg-green-100 text-green-700 border-green-200";
  ...
  const formattedDate = match.match_date ? new Date(match.match_date).toLocaleDateString('id-ID', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
    year: 'numeric'
      ...
      <main className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          <div className="p-6 md:p-8 bg-gradient-to-b from-gray-50 to-white flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16 border-b border-gray-100">
            ...
            <div className="flex flex-col items-center flex-1 w-full max-w-[200px]">
              <div className="w-24 h-24 rounded-full bg-white shadow-md border border-gray-100 flex items-center justify-center text-3xl font-black text-gray-400 mb-4 overflow-hidden">
                {teamA?.contingent?.image_url || teamA?.contingent?.cloudinary_public_id ? (
                  <img 
                    src={teamA?.contingent?.image_url || `https://res.cloudinary.com/demo/image/upload/${teamA?.contingent?.cloudinary_public_id}`} 
                    alt={teamA?.contingent?.name} 
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <span>{teamA?.contingent?.name?.charAt(0) || "?"}</span>
                )}
              </div>
            ...
            <div className="flex flex-col items-center flex-1 w-full max-w-[200px]">
              <div className="w-24 h-24 rounded-full bg-white shadow-md border border-gray-100 flex items-center justify-center text-3xl font-black text-gray-400 mb-4 overflow-hidden">
                {teamB?.contingent?.image_url || teamB?.contingent?.cloudinary_public_id ? (
                  <img 
                    src={teamB?.contingent?.image_url || `https://res.cloudinary.com/demo/image/upload/${teamB?.contingent?.cloudinary_public_id}`} 
                    alt={teamB?.contingent?.name} 
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <span>{teamB?.contingent?.name?.charAt(0) || "?"}</span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-4">
              <h3 className="font-bold text-gray-800 flex items-center gap-2 overflow-hidden">
                ...
                <span className="truncate" title={`Daftar Pemain ${teamA?.contingent?.name || "Tim A"}`}>
                  Daftar Pemain {teamA?.contingent?.name || "Tim A"}
                </span>
              </h3>
              <span className="text-xs font-bold text-gray-500 bg-gray-200 px-2.5 py-1 rounded-full shrink-0">
                {teamA?.players?.length || 0} Pemain
              </span>
            </div>
            <ul className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
              {teamA?.players?.map((player: any) => (
                  ...
                  <div className="flex flex-col items-end">
                    {player.risk_lvl && (
                      <span className={`text-[10px] px-2 py-0.5 rounded-full border font-semibold flex items-center gap-1 ${getRiskBadge(player.risk_lvl)}`}>
                        {['hijau', 'rendah', 'low', 'not_yet'].includes(player.risk_lvl.toLowerCase()) ? <ShieldCheck size={10} /> : <ShieldAlert size={10} />}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-4">
              <h3 className="font-bold text-gray-800 flex items-center gap-2 overflow-hidden">
                <div className="w-2 h-6 bg-red-500 rounded-full shrink-0"></div>
                <span className="truncate" title={`Daftar Pemain ${teamB?.contingent?.name || "Tim B"}`}>
                  Daftar Pemain {teamB?.contingent?.name || "Tim B"}
                </span>
              </h3>
              <span className="text-xs font-bold text-gray-500 bg-gray-200 px-2.5 py-1 rounded-full shrink-0">
                {teamB?.players?.length || 0} Pemain
              </span>
            </div>
            <ul className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
              {teamB?.players?.map((player: any) => (
                <li key={player.id} className="p-4 hover:bg-gray-50 transition-colors flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-red-50 text-red-600 flex items-center justify-center font-bold text-sm overflow-hidden border border-red-100">
                      {player.photo_path ? (
                         <img src={player.photo_path} alt={player.name} className="w-full h-full object-cover" />
                      ) : (
                         player.name?.charAt(0) || "?"
